...
- 根据 `modify_time` 距今时长及设备 `status` 字段，判断六级网络状态（在线 / 离线预警 / 离线 / 未激活 / 已停用 / 待安装）
...
├── test_regression.py     # 全链路回归测试（7 个模块，43 个用例）
└── README.md
```
...
> - 补录设备将追加到报表主表末尾，`油品型号` 显示为"**设备待安装调试且未录入系统**"，`网络同步` 显示 **待安装**（灰色）。
...
process_data()
    ...
    ├─ 4. 按安装时间升序排列
    ├─ 5. 补全桶数、设备归属字段
    ├─ 6. check_sync()  ← device_status + modify_time + is_stopped 判断六级网络状态
    ├─ 7. 数据清洗（空值填充）
...
| 优先级 | 条件 | 状态值 | 报表颜色 |
|--------|------|--------|----------|
| 1 | `t_device.status == 1`（设备已停用），或离线超72h且配置标记为"已停用" | `已停用` | 灰色 |
| 2 | `modify_time` 为空（从未上报数据） | `未激活` | 紫色 |
| 3 | `modify_time` 距今 **> 72 小时** | `离线` | 红色 |
| 4 | `modify_time` 距今 **≥ 18 且 ≤ 72 小时** | `离线预警` | 橙色 |
| 5 | `modify_time` 距今 **< 18 小时** | `在线` | 绿色 |
| 6 | 补录设备（待安装调试） | `待安装` | 灰色 |
...
| 模块 | 用例数 | 验证内容 |
|------|--------|---------|
| `check_sync` 六级状态 | 12 | 已停用 / 未激活 / 在线 / 离线预警 / 离线 / 待安装 全覆盖（含 is_stopped 覆盖 + 边界值） |
...
> **后续 Bugfix 与功能增强：**
> - **v1.0.1**：修复补录设备「已入库」判断漏洞。原逻辑以排除后的报表编号集（`existing_codes`）判断补录设备是否入库，导致「在DB但被排除」的设备被误判为未入库，以 `已停用` 状态追加到报表末尾。修复后改用全量 `_db_codes` 判断，排除设备不会再被重复追加。
...
| 10 | 六级网络状态判断 | 在线 / 离线预警 / 离线 / 未激活 / 已停用 / 待安装 均正确着色 | ✅ 分布：在线 119 / 离线 49 / 未激活 7，条件格式精确匹配（无 离线预警 误触发 离线 规则） |
| 11 | 补录设备追加 | 不在DB的补录设备追加至报表末尾，油品型号显示指定文案 | ✅ 1 台追加成功，油品型号显示"设备待安装调试且未录入系统"，网络同步显示 待安装 |
...
| 15 | 旧文件清理 | 保留修改时间在 10 天以内的报表及日志，超期按先进先出原则自动删除 | ✅ 清理逻辑正常执行 |
